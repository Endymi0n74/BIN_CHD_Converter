...
fn extensions(mode: &str) -> &'static [&'static str] { if mode == "convert" { &["cue","gdi","toc","iso","img","raw","cso","ciso","zip","7z","rar"] } else { &["chd"] } }
...
fn prepared_inputs(input: &Path) -> Result<(Vec<PathBuf>, Option<TempDir>), String> {
    ...
    if ext=="cso" || ext=="ciso" { let tool=find_command(&["maxcso"]).ok_or("CSO support requires maxcso in PATH or beside the app")?; let temp=TempDir::new().map_err(|e|e.to_string())?; let iso=temp.path().join(format!("{}.iso",input.file_stem().unwrap_or_default().to_string_lossy())); let result=Command::new(tool).arg("--decompress").arg(input).arg("-o").arg(&iso).output().map_err(|e|e.to_string())?; if !result.status.success(){return Err(String::from_utf8_lossy(&result.stderr).into_owned());} return Ok((vec![iso],Some(temp))); }
