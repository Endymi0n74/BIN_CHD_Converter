...
die() { printf 'Error: %s\n' "$*" >&2; exit 1; }
ACTION="${1:-}"; [[ -z "$ACTION" || "$ACTION" == --help || "$ACTION" == -h ]] && { usage; exit 0; }
...
convert_one() {
  local f="$1" rel ext base cmd out; rel="$(relative "$f")"; ext="$(printf '%s' "${f##*.}" | tr '[:upper:]' '[:lower:]')"; base="${rel%.*}"; out="$OUTPUT/$base.chd"; mkdir -p "$(dirname "$out")"
  case "$ext" in cue|gdi|toc) cmd=createcd;; iso) cmd=createdvd;; img) cmd=createhd;; raw) cmd=createraw;; *) return;; esac
  ((FORCE_CD)) && cmd=createcd; ((FORCE_DVD)) && cmd=createdvd; echo "CONVERT $rel"
  if run chdman "$cmd" -i "$f" -o "$out"; then ((DELETE && !DRY)) && rm -f -- "$f"; else rm -f -- "$out"; return 1; fi
}
extract_one() {
  local f="$1" rel base cmd suffix info; rel="$(relative "$f")"; base="${rel%.*}"; mkdir -p "$OUTPUT/$(dirname "$base")"
  ...
  echo "EXTRACT $rel"; if run chdman "$cmd" -i "$f" -o "$OUTPUT/$base.$suffix"; then ((DELETE && !DRY)) && rm -f -- "$f"; else return 1; fi
}
verify_one() { echo "VERIFY $(relative "$1")"; run chdman verify -i "$1"; }
export INPUT OUTPUT FORMAT FORCE_CD FORCE_DVD DELETE DRY; export -f run relative convert_one extract_one verify_one
