...
namespace FormatHelper.Formats.Mds;
...
internal static class MdsInputPreparer
{
    ...
    )
    {
        ...
        if (volumeSet is not null)
        {
            ...
            dataFilePath = Path.Combine(
                workDir,
                Path.GetFileNameWithoutExtension(disc.MdsPath) + ".bin"
            );
        ...
        if (disc.NeedsSubchannelStrip)
        {
            var strippedPath = Path.Combine(
                workDir,
                Path.GetFileNameWithoutExtension(dataFilePath) + ".stripped" + ".bin"
            );
            onLog?.Invoke(
    ...
    )
    {
        ...
        var cuePath = Path.Combine(
            workDir,
            Path.GetFileNameWithoutExtension(disc.MdsPath) + ".cue"
        );
